...
public class ShipWaterPhysics : FakeWaterPhysicsBase
{
    ...
    private void PlaySwimmingAnimations()
    {
        if (!isMoving)
        {
            if (!isPlayer) StartCoroutine(Move());

            else
            {
                if (isMovingUp) StartCoroutine(Move2(false));
                else StartCoroutine(Move2(true));
            }
        }
    ...
    private IEnumerator Move2(bool isUp)
    {
        isMoving = true;
